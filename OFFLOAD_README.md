...
- `--int8-quant-mode`: Int8 quantization strategy (`channel` for per-channel, `tensor` for per-tensor)
...
Set `--share-adapter-rank` to a small value (e.g., 32) to attach low-rank residual adapters to the shared blocks, with `--share-adapter-scale` controlling their contribution. Adapter energy statistics are reported under `adapter_energy` in the collected metrics. For int8 caching, prefer `--int8-quant-mode channel` (default) for best fidelity; switch to `tensor` only when transfer bandwidth dominates and slight quality loss is acceptable.
