...
1. **Weight streaming** – the offload planner keeps only the most critical blocks resident; the rest are prefetched from CPU when `--rabbit-offload` is enabled. Budgets (`--rabbit-memory-budget-mb`) now evict the heaviest blocks first while respecting a per-stage floor (`--rabbit-min-device-blocks`, default 1) so the runtime can target much tighter VRAM envelopes even on small GPUs. Prefetch depth is controllable via `--rabbit-prefetch-distance`.
...
>
> 1. **Weight locality planner** – we partition the DiT blocks into resident and streaming sets. Blocks are ranked by parameter footprint and moved to host memory until the requested device budget (`--rabbit-memory-budget-mb`) is satisfied, keeping only `--rabbit-min-device-blocks` (default 1) per stage on the GPU. Prefetching (`--rabbit-prefetch-distance`) controls how many streamed blocks are staged concurrently, and the ProfilingDiT-style warmup (`--rabbit-profile-steps`) can still bias the plan toward background-heavy layers automatically.
