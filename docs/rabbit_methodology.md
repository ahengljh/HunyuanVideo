...
> 3. **Output caching** – blocks with low EMA importance capture their output tensors (both image and text streams) and a lightweight signature. When the signature drift stays below `rabbit_cache_threshold`, we re-inject the cached result instead of recomputing, avoiding quality loss that arises from identity skips.
>
